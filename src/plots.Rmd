---
title: "Plot draft and valley crossing simulation results"
---
...
p.ctime = grid %>% filter(s==0.1) %>% group_by(K, delta, sigma, s) %>% summarize(m_crossing_time = log10(mean(crossing_time))) %>% 
  ggplot(aes(x=delta, y=K)) + 
  geom_tile(aes(fill=m_crossing_time, color=m_crossing_time)) + 
  ...
  facet_grid(.~sigma, labeller = label_bquote(cols=sigma==.(fancy_scientific(sigma)[[1]])))
p.ctime
...
p.logratio = gs %>% filter(s==0.1, sval!=0.001) %>%
  ...
  scale_x_log10(labels=tenx_format) + scale_y_log10(labels=tenx_format) + add_theme_elements + 
  labs(x = expression(delta), y = expression(italic(N))) +
  scale_fill_div_plot(name = expression(log[10](italic(alpha[sigma])/italic(alpha[5 %*% 10^-8])))) + scale_color_div_plot(guide=FALSE) +
  facet_grid(sigma~sval, labeller = label_bquote(rows=sigma==.(fancy_scientific(sigma)[[1]]), cols=italic(s)[sweep]==.(fancy_scientific(sval)[[1]])))
p.logratio
...
p.ss = ss %>%
  ...
  facet_grid(.~mu1, labeller = label_bquote(cols=italic(mu[1])==.(fancy_scientific(mu1)[[1]])))
p.ss
