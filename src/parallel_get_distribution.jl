...
function main(args)
    ...
    defpars = Dict{String,Any}([
        ...
        "num_attempts" => Dict("value" => 100, "type" => Int64),
        "num_loci" => Dict("value" => 10, type => Int64),
        "burn_factor"   => Dict("value" => 0.1, "type" => Float64)
    ])
    ...
    @everywhere function run_worker(inputs, results)
        ...
        while true
            ...
            segregating_clones = []
            attempt_num = 0
            clone_sizes = zeros(num_attempts)
            clone_lifetimes = zeros(num_attempts)
            clone_fitnesses = zeros(num_attempts)
            while attempt_num < num_attempts
                while length(segregating_clones) < min(num_loci, num_attempts - attempt_num)
                    tmp_clone_data = inject_mutation!(pop, attempt_num)
                    push!(segregating_clones, [attempt_num, tmp_clone_data[1], tmp_clone_data[2], pop.generation])
                    attempt_num += 1
                end
                evolve_multi!(pop)
                for cl_info in segregating_clones
                    cl_id = cl_info[1]
                    clone_size = sum([cl.n for cl in get_mutated_classes(pop, cl_id)])
                    clone_sizes[cl_id] += clone_size
                    if clone_size == 0
                        pop!(segregating clones, cl_info)
                        clone_lifetimes[cl_id] = pop.generation - cl_info[4]
                        clone_fitnesses[cl_id] = cl_info[3]
                    end
                end
            end
